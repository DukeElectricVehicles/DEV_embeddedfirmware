...
void setupADC(){
	...
	PDB0_CH0DLY0 = 100; // Almost immediately trigger the first ADC conversion
...
uint16_t getThrottle_ADC() {
	...
	uint16_t toRet = thr_cnts >> (ADC_RES_BITS-10); // analog read res is 12 bit
	if (toRet > (1<<10)){
		return prevThrot;
	}
	prevThrot = toRet;
...
void adc_isr() {
  ...
  if (ADCsampleCollecting) {
  	vsxSamples_cnts[vsxSample_ind][0] = vsx_cnts[0];
  	vsxSamples_cnts[vsxSample_ind][1] = vsx_cnts[1];
  	vsxSamples_cnts[vsxSample_ind][2] = vsx_cnts[2];
